...
SEED = 42
NON_ALPHA = re.compile("[^A-Za-z_0-9]") # 匹配非字母、非数字、以及非下划线的字符
RNG = np.random.RandomState(SEED)
MAX_HASH = np.uint64((1 << 32) - 1) # 32位无符号整数的最大值
MERSENNE_PRIME = np.uint64((1 << 61) - 1)   # 梅森素数 2^61-1
logger = logging.getLogger(__name__)    
logger.setLevel(logging.INFO)
datasets.logging.set_verbosity_error()  


def ngrams(sequence: List[str], n: int, min_ngram_size: int):
...
def embed_func(
    ...
    *,
    ...
    tokens = {" ".join(t) for t in ngrams(NON_ALPHA.split(content.strip()), ngram_size, min_ngram_size)}
...
if __name__ == "__main__":
    def run(
        dataset: str = typer.Option("xxx.json", help="Path of the dataset"),  
        column: str = typer.Option("", help="Dataset column to deduplicate"),
        ...
        output: str = typer.Option("", help="Store the deduplicated dataset"),
    ):
        global uf
        OUTPUT_BASE = Path(output or "output")
        OUTPUT_BASE.mkdir(exist_ok=True, parents=True)
        output = OUTPUT_BASE / f"deduplicated_{threshold}"
